 ...
 *
 *
 * mod_xpass.c -- Socket Controlled Event Handler
 *
 */
#include "mod_xpass.h"

#define XPASS_DESC "xpass information"
#define XPASS_SYNTAX "<command> [<args>]"

globals_t globals;

SWITCH_MODULE_LOAD_FUNCTION(mod_xpass_load);
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_xpass_shutdown);
SWITCH_MODULE_RUNTIME_FUNCTION(mod_xpass_runtime);
SWITCH_MODULE_DEFINITION(mod_xpass, mod_xpass_load, mod_xpass_shutdown, mod_xpass_runtime);
...
SWITCH_DECLARE_GLOBAL_STRING_FUNC(set_pref_xpass_var_prefix, globals.xpass_var_prefix);
...
static switch_status_t api_erlang_event_filter(switch_stream_handle_t *stream) {
	...
	stream->write_function(stream, "%-50s", globals.xpass_var_prefix);
...
static switch_status_t config(void) {
	...
	if (zstr(globals.xpass_var_prefix)) {
		...
		char *buf;
		int size = switch_snprintf(NULL, 0, "variable_%s*", globals.xpass_var_prefix) + 1;
		
		switch_malloc(buf, size);
		switch_snprintf(buf, size, "variable_%s*", globals.xpass_var_prefix);
		switch_safe_free(globals.xpass_var_prefix);
		globals.xpass_var_prefix = buf;
